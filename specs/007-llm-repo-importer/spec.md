...
- **FR-006**: Identify connections that span two or more repositories (e.g., a message queue topic produced by one repository and consumed by another) that are not visible from any single repository's own knowledge graph, using a hybrid strategy: a deterministic matcher runs first, an agentic reasoning pass over condensed per-repo summaries runs only for the repo pairs the deterministic matcher could not resolve. _(Revised 2026-08-22)_ The deterministic matcher comprises six passes — five evidence-grounded passes over raw repository source (package manifests, HTTP endpoint/URL-literal matching incl. gateway-prefixed variants, schema digests, docker-compose wiring incl. well-known external systems, pub/sub topic literals), ported from the understand-everything linker core, plus the original graph-text name-mention pass — each connection carrying human-readable evidence naming what produced it
