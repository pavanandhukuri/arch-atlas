...
export function wizardReducer(state: WizardState, action: WizardAction): WizardState {
  switch (action.type) {
    case 'LOAD_REVIEW': {
      const preloadedSystems: SystemGroup[] = action.file.systems.map((sg) => ({
        id: sg.name
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, '-')
          .replace(/^-+|-+$/g, ''),
    ...
    case 'NEXT_STEP': {
      const nextStep = Math.min(state.step + 1, 6) as WizardStep;
    ...
    case 'SET_ELEMENT_REVIEWED': {
      return {
        ...state,
        elements: state.elements.map((el) =>
          el.id === action.id ? { ...el, reviewed: action.reviewed } : el
        ),
      };
    }

    case 'ACCEPT_ALL_ELEMENTS': {
      return {
        ...state,
        elements: state.elements.map((el) => ({ ...el, reviewed: true })),
      };
    }

    case 'SET_ELEMENT_FILTER': {
      return { ...state, elementFilter: action.filter };
    }

    case 'BULK_ASSIGN_SYSTEM': {
      const ids = new Set(action.ids);
      return {
        ...state,
        elements: state.elements.map((el) =>
          ids.has(el.id) && el.kind === 'container'
            ? { ...el, systemId: action.systemId, reviewed: true }
            : el
        ),
      };
    }

    case 'BULK_MARK_EXTERNAL': {
      const ids = new Set(action.ids);
      return {
        ...state,
        elements: state.elements.map((el) =>
