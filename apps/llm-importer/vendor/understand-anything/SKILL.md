...
1.  **Resolve `PROJECT_ROOT`:**
    ...
    - **Worktree redirect.** If `PROJECT_ROOT` is inside a git worktree (not the main checkout), redirect output to the main repository root. Worktrees managed by Claude Code are ephemeral — the data directory (`.ua/`, or legacy `.understand-anything/`) written there is destroyed when the session ends, taking the knowledge graph with it (issue #133). Detect a worktree by comparing `git rev-parse --git-dir` against `git rev-parse --git-common-dir`; in a normal checkout or submodule they resolve to the same path, in a worktree they differ and the parent of `--git-common-dir` is the main repo root.
      ...
      1.5. **Resolve `SKILL_DIR`.** _(arch-atlas headless patch — replaces UA's original plugin-root discovery/build step.)_ The original step here searched host-specific install locations (`~/.agents/skills/understand`, `~/.understand-anything-plugin`, etc.) and built `@understand-anything/core` if missing. Neither applies: this skill is vendored at a fixed, known path inside the arch-atlas importer package (`apps/llm-importer/vendor/understand-anything/`) and none of the scripts it invokes depend on `@understand-anything/core` (research.md D4 — that package pulls in a dozen native/WASM tree-sitter grammars for import-graph batching and file-fingerprinting our pipeline doesn't use; `compute-batches.mjs` and `generate-ignore.mjs` are plain, dependency-free reimplementations instead). `SKILL_DIR` is passed explicitly by the launching session (`src/analysis/resource-loader.ts`) — do not attempt to derive it from a symlink or search host directories.
...
Set up the `.understandignore` file before scanning.

> **[arch-atlas headless patch]** This phase runs unattended — there is no human in the loop
> for a batch/CI import. Generate the file and proceed immediately; do not wait for
> confirmation. (research.md D4 adaptation 1)
...
2. **If it does NOT exist**, generate a starter file by invoking the bundled script (delegates to `generateStarterIgnoreFile` in `@understand-anything/core`, which reads `.gitignore`, deduplicates against built-in defaults, and emits language-grouped test-file suggestions). Pass `$PLUGIN_ROOT` via the env so the script doesn't have to re-derive it from its own path (which breaks for copied skill installs):
   ...
   - Report: `Generated $UA_DIR/.understandignore with suggested exclusions based on your project structure.`
3. **If it already exists**, report: `Found $UA_DIR/.understandignore.`
4. Proceed immediately to Phase 1 — no confirmation wait.
...
Report: `[Phase 2/6] Analyzing files — <totalFiles> files in <totalBatches> batches (up to <maxConcurrency> concurrent)...`
...
## Phase 4/5 — ARCHITECTURE / TOUR (removed)

**[arch-atlas headless patch — Phases 4 and 5 trimmed entirely, research.md D4.]** UA's original Phase 4 dispatches an `architecture-analyzer` agent to assign nodes to architectural layers; Phase 5 dispatches a `tour-builder` agent to produce a guided walkthrough. Both exist to power UA's own interactive dashboard. The arch-atlas review wizard consumes neither `layers` nor `tour`, and generating them costs two full additional agent dispatches' worth of local-model time per repository for output that would be discarded. Proceed directly from Phase 3 to Phase 6, treating `layers` and `tour` as `[]` throughout.

The original text of these two phases is not reproduced here (see `apps/llm-importer/README.md` for the pinned upstream commit to diff against on re-sync) — inlining disabled instructions in a prompt file the agent reads is unsafe, since a model may still act on text inside a comment marker.

---

## Phase 6 — REVIEW

Report to the user: `[Phase 4/6] Validating knowledge graph...`
...
_(arch-atlas headless patch: `layers`/`tour` are always `[]` — Phases 4/5 that would populate them are trimmed, research.md D4. The validation checks in step 1 below still apply; they pass trivially against empty arrays.)_
...
Report to the user: `[Phase 5/6] Saving knowledge graph...`

1. Write the final knowledge graph to `$UA_DIR/knowledge-graph.json`.

2. **[arch-atlas headless patch — fingerprint baseline generation removed, research.md D4.]** UA's original step here builds a structural-fingerprint baseline (via `build-fingerprints.mjs`, which depends on `@understand-anything/core`'s tree-sitter grammars — not vendored) so its own auto-update mechanism can later tell which files changed without a full re-scan. arch-atlas doesn't use UA's auto-update mechanism: incremental re-import is decided at the repository level, by whether a valid `{repo}.knowledge-graph.json` artifact already exists (spec.md FR-011, US3) — a coarser but dependency-free check. Skip straight to step 3.

3. Write metadata to `$UA_DIR/meta.json`:
...
5. Report a summary containing:
   ...
   _(arch-atlas headless patch: "Layers identified" and "Tour steps generated" removed from this summary — Phases 4/5 that produced them are trimmed from this phase sequence entirely, research.md D4.)_

6. **[arch-atlas headless patch — dashboard auto-launch removed, research.md D4.]** UA's original step here launches an interactive dashboard. There is no interactive session to launch it in; `src/analysis/run-understand.ts` copies `knowledge-graph.json` out of `$UA_DIR` after this phase completes (research.md D4 adaptation 3) instead.
