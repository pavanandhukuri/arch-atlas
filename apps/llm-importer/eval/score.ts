 ...
 *
 * Ported from `packages/analysis-runner-local/eval/score.ts` (git dda7094) with
 * only the type imports repointed at the in-package correlator, plus the new
...
export const EXTRACTION_FIELDS = Object.keys(FIELD_MATCHERS);
...
// --- external-system scoring -------------------------------------------
//
// A "cross-repo connection" whose target is NOT a workspace repo IS an external
// system (an IdP, an object store, a managed queue). Score how many of the
// ground-truth externals the correlation recovered. Recall is the headline —
// a dropped external dependency is the regression we care about — but precision
// is reported alongside to catch the normalization dictionary over-firing.

export function scoreExternalSystems(
  connections: CrossRepositoryConnection[],
  expected: string[],
  workspaceRepoNames: string[]
): PRF {
  const repoSet = new Set(workspaceRepoNames.map(normalize));
  const predicted = [
    ...new Set(connections.map((c) => c.targetRepo).filter((t) => !repoSet.has(normalize(t)))),
  ];
  return prf(predicted, expected, nameMatch);
}
