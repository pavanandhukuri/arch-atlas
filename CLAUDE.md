...
- TypeScript 5.3.0 strict, Node.js ≥ 22. **010**: `apps/llm-importer` core goes deterministic + model-free (drops `@earendil-works/pi-*` + `typebox`); the per-repo `RepoAnalysis` is produced by an external, swappable producer — `plugins/repo-analysis`, an `AGENTS.md` procedure any coding agent can run against a local or hosted model of the developer's choosing (no in-repo code calls a model). New artifacts: `{repo}.context.json`, optional `architecture.extra-connections.json`. Update: the original local-model reference runner, `packages/analysis-runner-local`, has since been deleted — see the `chore/drop-local-model-producer` cleanup. (010-harness-neutral-importer)
