...
- **analysis-runner-local**: Reference producer for `apps/llm-importer` — scans a repository via a local, OpenAI-compatible model endpoint and writes `{repo}.analysis.json` (offline, no hosted API). One of two producers implementing the same contract; the other is the `.claude/skills/repo-analysis` Claude Code skill.
